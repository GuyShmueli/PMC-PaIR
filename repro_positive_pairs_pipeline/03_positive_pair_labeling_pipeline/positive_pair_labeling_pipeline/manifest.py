...
from .config import PIPELINE_ID, PIPELINE_VERSION, PipelineConfig
from .errors import ArtifactIntegrityError, StageOrderError
...
def implementation_metadata() -> dict[str, Any]:
    ...
    source_paths = sorted(
        [*project_root.glob("positive_pair_labeling_pipeline/*.py"), *project_root.glob("scripts/*.py")],
    ...
    for name in (
        ...
        if not path.is_file():
            raise ArtifactIntegrityError(f"Implementation contract file is missing: {path}")
        source_paths.append(path)
    files = {
        path.relative_to(project_root).as_posix(): sha256_file(path)
        for path in sorted(
            set(source_paths), key=lambda item: item.relative_to(project_root).as_posix()
        )
    }
...
def load_manifest(output_dir: str | Path) -> dict[str, Any]:
    ...
    if manifest.get("pipeline_version") != PIPELINE_VERSION:
        raise ArtifactIntegrityError("Run was created by a different pipeline version")
    return manifest
...
def require_stage(
    ...
    *,
    ...
    if not isinstance(stage_record, dict) or stage_record.get("status") not in statuses:
        raise StageOrderError(
            f"Stage {stage!r} must have status {statuses}; run the preceding documented step first"
        )
    return stage_record
...
def verify_manifest(output_dir: str | Path) -> dict[str, Any]:
    ...
    if manifest.get("prompts") != prompt_metadata():
        raise ArtifactIntegrityError(
            "Prompt implementation changed after this run was initialized"
        )
    ...
    expected_spec = {
        ...
        "selection_handoff": manifest["selection_handoff"],
        "prompts": manifest["prompts"],
    }
    if sha256_bytes(canonical_json_bytes(expected_spec)) != manifest.get("run_spec_id"):
    ...
    if not isinstance(artifacts, dict) or not isinstance(stages, dict):
        raise ArtifactIntegrityError("Run manifest artifact/stage tables are malformed")
    for stage_name, stage_record in stages.items():
        if not isinstance(stage_record, dict) or not isinstance(
            ...
            raise ArtifactIntegrityError(f"Stage record is malformed: {stage_name}")
        for relative in stage_record["artifacts"]:
            if relative not in artifacts:
                raise ArtifactIntegrityError(
                    f"Stage {stage_name!r} references unrecorded artifact {relative!r}"
                )
